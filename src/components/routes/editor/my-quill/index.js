...
class MyQuill extends Component {
  ...
  componentDidMount() {
    ...
    const butCloudSync = this.toolbarRef.querySelector('#butCloudSync');
    butCloudSync.addEventListener('click', async () => {
      if (this.props?.onResync) {
        this.props.onResync();
      }
    });
      ...
      <div class={style.fullContainer} ref={el => { this.containerRef = el }}>
        <div class={style.toolbarContainer} ref={el => { this.toolbarRef = el }}>
          ...
          <span class="ql-formats">
            ...
            {/* Download from cloud */}
            <button id="butCloudSync" style="display:none;" tabindex="3" aria-label="download" title="Sync from Cloud">
              <svg xmlns="http://www.w3.org/2000/svg" height="18px" viewBox="0 0 24 24" width="18px" class="ql-fill">
                <path d="M0 0h24v24H0V0z" fill="none" />
